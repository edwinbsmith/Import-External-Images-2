...
define( 'SI_EXTERNAL_IMAGES_DIR', plugin_dir_path( __FILE__ ) );
define( 'SI_EXTERNAL_IMAGES_URL', plugins_url( basename( dirname( __FILE__ ) ) ) );

define( 'SI_EXTERNAL_IMAGES_ALLOW_BULK_MESSAGE', FALSE );
...
$posts_count_custom = get_option( 'si_external_image_posts_count_custom', '500' );
...
add_action( 'admin_head', 'si_external_images_bulk_resize_admin_javascript' );
add_action( 'admin_notices', 'si_external_images_bulk_resize_message', 90 );
...
function si_external_image_admin_init() {
	...
	register_setting( 'si_external_image', 'si_external_image_whichimgs', 'si_external_image_images_count_custom', 'si_external_image_posts_count_custom' );

	add_option( 'si_external_image_images_count_custom', '50' );
...
function si_external_images_bulk_resize_message() {
	global $pagenow;

	if ( SI_EXTERNAL_IMAGES_ALLOW_BULK_MESSAGE ) {
		...
		if ( $pagenow == 'upload.php' && isset( $_GET['page'] ) && $_GET['page'] == 'si_external_image' ) {
 ...
 **/
function si_external_images_bulk_resize_admin_javascript() {
	echo "<script type=\"text/javascript\" src=\"" . SI_EXTERNAL_IMAGES_URL . "/import-external-images.js\" ></script>\n";
}
 ...
 **/
function si_external_image_get_backcatalog_ajax() {
	$posts_to_import = si_external_image_get_backcatalog();
 ...
 * Used in import-external-images.js
 **/
function si_external_image_import_all_ajax() {
	...
	$response  = si_external_image_import_images( $post_id, TRUE );
...
function si_external_image_menu() {
	add_media_page( 'Import Images', 'Import Images', 'edit_theme_options', 'si_external_image', 'si_external_image_options' );
}
...
function si_external_image_import_images( $post_id, $force = FALSE ) {
	...
	$imgs                = si_external_image_get_img_tags( $post_id ); // Array of external image URLs
	$images_count_custom = get_option( 'si_external_image_images_count_custom', 200 );
...
function si_external_image_get_img_tags( $post_id ) {
	$post = get_post( $post_id );
	$w    = get_option( 'si_external_image_whichimgs' );
	$s    = home_url();

	$excludes = get_option( 'si_external_image_excludes' );
...
function si_external_image_backcatalog() {

	$numberposts = get_option( 'si_external_image_posts_count_custom', '500' );
	...
	$before   = '<form style="padding: 10px; margin: 20px 20px 0 0; float: left;" action="" method="post" name="si_external_image-backcatalog">';
	...
	foreach ( $posts as $post ) {
		...
		try {
			$imgs = si_external_image_get_img_tags( $post->ID );
			if ( is_array( $imgs ) && count( $imgs ) > 0 ) {

				$count += count( $imgs );

				echo '<p>Post titled: "<strong>' . $post->post_title . '</strong>" - ';
				si_external_image_import_images( $post->ID, TRUE );
...
function si_external_image_get_backcatalog() {
...
function si_external_image_options() {
	$_cats  = '';
	$_auths = '';
	?>
    ...
    <div class="wrap"
         style="overflow:hidden; display:inline-block; height:100%;height">
	...
	if ( isset( $_POST['action'] ) && $_POST['action'] == 'backcatalog' ) {
		...
		si_external_image_backcatalog();
		echo '</div>';

	} elseif ( isset( $_POST['action'] ) && $_POST['action'] == 'update' ) {
		update_option( 'si_external_image_whichimgs', esc_html( $_POST['si_external_image_whichimgs'] ) );
		update_option( 'si_external_image_excludes', esc_html( $_POST['si_external_image_excludes'] ) );
		update_option( 'si_external_image_images_count_custom', esc_html( $_POST['si_external_image_images_count_custom'] ) );
		update_option( 'si_external_image_posts_count_custom', esc_html( $_POST['si_external_image_posts_count_custom'] ) );
    ...
    <form name="si_external_image-options" method="post" action=""
          style="padding: 20px; margin: 20px 20px 0 0 ; float: left; display:block; background: #f6f6f6; border: 1px solid #e5e5e5; ">
		<?php settings_fields( 'si_external_image' ); ?>

        <h2>Options</h2>
        <div style="width: 50%;display:inline;float:left;">
            <p class="howto">We will download externally hosted images found
                within this site's posts so that those images are hosted locally
                on this site's web server. Links to externally hosted images
                will be updated by this process too.</p>
            <h3>How many images and posts to process</h3>
            <p>The import process might stop if there are too many images and
                posts to process. Select lower values to process per run to
                improve the import process.</p>
            <p><label for="si_external_image_images_count_custom">Images per
                    Post</label><br>
                <input type="number"
                       name="si_external_image_images_count_custom" min="1"
                       max="200"
                       value="<?php echo( get_option( 'si_external_image_images_count_custom' ) ); ?>">
            </p>
            <p class="howto">Default is 200. Maximum is 200.</p>

            <p><label for="si_external_image_posts_count_custom">Posts per
                    Run</label><br>
                <input type="number" name="si_external_image_posts_count_custom"
                       min="1" max="500"
                       value="<?php echo( get_option( 'si_external_image_posts_count_custom' ) ); ?>">
            </p>
        ...
        <div style="width: 50%;display:inline;float:left;">
            ...
            <p>By default, all images hosted on any external site are processed.
                Use these options to ignore images from certain domains.</p>
            <p>
                <label for="myradio1">
                    <input id="myradio1" type="radio"
                           name="si_external_image_whichimgs"
                           value="all" <?php echo( get_option( 'si_external_image_whichimgs' ) != 'exclude' ? 'checked="checked"' : '' ); ?> />
                    All images
                </label>
            </p>
            ...
            <p>
                <label for="myradio2">
                    <input id="myradio2" type="radio"
                           name="si_external_image_whichimgs"
                           value="exclude" <?php echo( get_option( 'si_external_image_whichimgs' ) == 'exclude' ? 'checked="checked"' : '' ); ?> />
                    Exclude images by domain
                </label>
            </p>

            <p><label for="myradio2">Domains to exclude (comma
                    separated):</label></p>
            <p class="howto">Example: smugmug.com, flickr.com, picassa.com,
                photobucket.com, facebook.com</p>
            <p><textarea style="height:90px; width: 90%;"
                         id="si_external_image_excludes"
                         name="si_external_image_excludes"><?php echo( get_option( 'si_external_image_excludes' ) != '' ? get_option( 'si_external_image_excludes' ) : '' ); ?></textarea>
            </p>

            <div class="submit">
                <input type="hidden" name="si_external_image_update"
                       value="action"/>
    ...
    <div id="si_import_all_images"
		...
		foreach ( $posts as $this_post ) {
			$images = si_external_image_get_img_tags( $this_post->ID );
			if ( ! empty( $images ) ) {
		...
		if ( ! empty( $posts_to_fix ) ) {
			if ( is_array( $posts_to_fix ) ) {
				...
				$html .= '<p class="howto">You should <a class="button-secondary" href="' . admin_url( 'upload.php?page=si_external_image' ) . '">refresh the page</a> when done to check if you have more posts to process.</p>';
